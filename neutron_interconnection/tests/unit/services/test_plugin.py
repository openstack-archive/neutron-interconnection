...
from neutron_interconnection import extensions as inter_ext
...
from neutron_interconnection.services import plugin

NEUTRON_INTERCONNECTION_PLUGIN_KLASS = (
    "neutron_interconnection.services.plugin.NeutronInterconnectionPlugin"
)
TEST_L3_PLUGIN_KLASS = (
...
class TestNeutronInterconnectionPlugin(
    ...
    def setUp(self):
        ...
        service_plugins = {
            'neutron_interconnection': NEUTRON_INTERCONNECTION_PLUGIN_KLASS,
            'l3_plugin': TEST_L3_PLUGIN_KLASS
        }
        extensions_path = ':'.join(inter_ext.__path__ + n_extensions.__path__)

        # we need to provide a plugin instance, although
        # the extension manager will create a new instance
        # of the plugin
        self.interconnection_plugin = plugin.NeutronInterconnectionPlugin()
        ext_mgr = api_extensions.PluginAwareExtensionManager(
            extensions_path,
            {inter_api_def.ALIAS: self.interconnection_plugin,
